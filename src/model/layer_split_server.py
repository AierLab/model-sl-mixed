...
class SplitServerLayer(AbstractModel):
    def __init__(self, model_dir: str, in_queue: Queue, out_queue: Queue, skip, device=None, first_layer=False, last_layer=False):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.skip:
            print("Skip split layer.")
            return x

        print("Enter split layer")
